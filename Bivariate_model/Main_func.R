...
lp.approx3 <- nlm(p=Theta.init2,samp_no=samp_no,dataX=log(Data.ann2[,c(7,8,21)]),dist.Y=dist.Y,dataY=data.frame(Data.ann2[,c(14)]),z=Z, f=NLog_Like_Res1, hessian=TRUE,iterlim=400)
min_lp11=lp.approx3$minimum
...
data1=data.frame(Station=as.factor(pred.site$site_no),X=pred.site$site_name,MY1,SY1,MY1U,SY1U,MY2U,SY2U,MY2,SY2,Y1=Act.Y[,1],Y2=Act.Y[,2])

vec.name=paste("Results/out",j,".RData",sep="")
